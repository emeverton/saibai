...
class CartItemsComponent extends HTMLElement {
  ...
  #handleCartError = (line, json) => {
    ...
    cartItemErrorContainer.removeAttribute('hidden');
  };
...
class CountryProvinceComponent extends HTMLElement {
  constructor() {
    ...
    if (!this.countryElement || !this.provinceElement) return;

    this.countryElement.addEventListener('change', this.handleCountryChange.bind(this));

    const lockCountry = (this.dataset.lockCountry || '').trim();
    const preferredCountry = lockCountry || this.dataset.country || '';

    if (preferredCountry !== '') {
      const idx = Array.from(this.countryElement.options).findIndex((option) => {
        const text = (option.textContent || '').trim();
        const value = (option.value || '').trim();
        return (
          text === preferredCountry ||
          value === preferredCountry ||
          (preferredCountry === 'Brazil' && (value === 'Brazil' || text === 'Brasil' || text === 'Brazil')) ||
          (preferredCountry === 'Brasil' && (value === 'Brazil' || text === 'Brasil' || text === 'Brazil'))
        );
      });
