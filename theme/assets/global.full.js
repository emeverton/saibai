...
(function () {
  window.deferUntilInteraction(function () {
...
theme.utils = {
  ...
  imageLoaded: (imgOrArray) => {
    ...
    (function () {
      ...
      async function getFirstAvailableVariantId(productUrl) {
        ...
        if (!response.ok) {
          throw new Error('product_json_failed');
        }

        const product = await response.json();
        const variants = Array.isArray(product && product.variants) ? product.variants : [];
        const available = variants.find((variant) => variant && variant.available);
        const fallback = variants[0];
        return (available && available.id) || (fallback && fallback.id) || null;
      }

      function syncCartUI(cartState) {
        ...
        document.dispatchEvent(new CustomEvent('cart:updated', { detail: cartState }));

        if (typeof publish === 'function' && typeof PUB_SUB_EVENTS !== 'undefined' && PUB_SUB_EVENTS.cartUpdate) {
          publish(PUB_SUB_EVENTS.cartUpdate, {
            source: 'saibai-plp-quick-add-fallback',
            cartData: cartState,
          });
        }
      }
      ...
      document.addEventListener(
        ...
        async function (event) {
          const button = event.target.closest(
            'modal-opener[data-modal="#QuickViewStandard"] .saibai-plp-quick__btn[data-open-quick-view-popup]'
          );

          if (!button) return;

          const opener = button.closest('modal-opener');
          const modalSelector = opener ? opener.getAttribute('data-modal') : null;
          const modalExists = !!(modalSelector && document.querySelector(modalSelector));

          // For this PLP button, force direct add-to-cart.
          // The QuickView pipeline can exist in DOM but fail to open/add.
          void modalExists;
          ...
          try {
            const variantId = await getFirstAvailableVariantId(productUrl);
            ...
            const cartState = await addVariantToCart(variantId);
            syncCartUI(cartState);
          } catch (error) {
            console.error('[Saibai] PLP quick add fallback failed:', error);
            window.location.href = productUrl;
          } finally {
...
class QuantityInput extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    const minAttr = parseInt(this.input.min, 10);
    const maxAttr = this.input.max !== "" ? parseInt(this.input.max, 10) : NaN;
    ...
    if (event.target.name === "plus") {
      ...
      } else if (!isNaN(maxAttr) && !isNaN(current) && current >= maxAttr) {
      ...
      if (!isNaN(current) && current < 0) {
        this.input.value = !isNaN(minAttr) ? minAttr : 1;
      } else {
        this.input.stepDown();
      }
    }

    if (!isNaN(minAttr) && parseInt(this.input.value, 10) < minAttr) {
      this.input.value = minAttr;
    }
    if (!isNaN(maxAttr) && parseInt(this.input.value, 10) > maxAttr) {
      this.input.value = maxAttr;
    }
