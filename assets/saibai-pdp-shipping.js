(function () {
  ...
  function renderResults(el, rates, error) {
    el.hidden = false;
    if (error) {
    ...
    if (!rates || !rates.length) {
      ...
      el.innerHTML = '<p>Não encontramos frete para este CEP. <a href="/pages/politica-de-entrega">Ver política de entrega</a></p>';
      return;
    }
      ...
      fetchCep(cep)
        .then(function (address) {
          return ensureCartLine(variantId, getQuantity(root)).then(function () {
            return tryRates(formatCep(cep), address.uf);
          });
        })
        .then(function (rates) {
          renderResults(results, rates, null);
        })
        .catch(function () {
